...
- The stage's default **On model** view (`components/suit/ModelPreview.tsx`) re-dyes studio photos:
  sources in `design/model-src/`, built by `python3 scripts/build-model-poses.py` into `public/model/` +
  `lib/suit/model-poses.json`; `choosePose()` picks the photo for a configuration.
