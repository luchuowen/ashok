...
- The stage's photo views (`components/suit/PhotoPreview.tsx`) re-dye Nano Banana Pro product photos:
  sources + prompts in `design/nano-banana/`, built by `python3 scripts/build-suit-photos.py` into
  `public/suit-photos/` + `lib/suit/photo-assets.json`; `choosePhoto()` maps a configuration to a photo
  (null → the Detail drawing).
